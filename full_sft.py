...
from src.models.datasets import SFTDataset
import logging
...
def train_epoch(epoch):
    ...
    for step, (input_ids, labels, attn_mask) in enumerate(train_loader):
        ...
        with ctx:
            logits = model(input_ids, labels).logits
        ...
        if (step + 1) % args.save_interval == 0 and (not ddp or dist.get_rank() == 0):
            ...
            auto_map = {
                ...
                "AutoModel": "model.MiniChat",
            }

            current_folder = os.path.dirname(os.path.realpath(__file__))
            shutil.copyfile(
                os.path.join(current_folder, "../../models/model.py"),
                os.path.join(current_folder, "model.py"),
            )
            shutil.copyfile(
                os.path.join(current_folder, "../../models/model_config.py"),
...
def init_model():
    ...
    model = AutoModelForCausalLM.from_pretrained("./minimind-v1-small")

    log(f"LLM总参数量：{count_parameters(model) / 1e6:.3f} 百万")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniChat Full SFT")
    parser.add_argument(
    ...
    args.wandb_run_name = f"MiniChat-Full-SFT-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LearningRate-{args.learning_rate}"
