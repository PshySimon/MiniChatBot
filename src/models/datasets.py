...
class PretrainDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        # 0表示不计算损失
        attn_mask = [1] * text_len + [0] * padding_len

        input_id = np.array(input_id)
        input_ids = np.array(input_id[:-1]).astype(np.int64)
        labels = np.array(input_id[1:]).astype(np.int64)
        attn_mask = np.array(attn_mask[1:]).astype(np.int64)
        return torch.from_numpy(input_ids), torch.from_numpy(labels), torch.from_numpy(attn_mask)
...
class SFTDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        # 0表示不计算损失
        attn_mask = [0] * question_length + [1] * (mask_len) + [0] * padding_len
        ...
        return input_ids, labels, attn_mask_tensor
